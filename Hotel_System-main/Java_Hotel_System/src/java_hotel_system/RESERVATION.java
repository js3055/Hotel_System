...
public class RESERVATION {
        ...
        //2 for the Room
        //-->ALTER TABLE reservations ADD CONSTRAINT fk_client_id FOREIGN KEY (client_id)REFERENCES clients(id) on DELETE CASCADE
        //and add another foreign key betweeb table types and rooms
        //--> ALTER TABLE rooms ADD CONSTRAINT fk_type_id FOREIGN KEY (type) REFERENCES type(id) on DELETE 
        
    
        //some fixes we need to do
        //1 - when we add new reservation the room associated with it should be set to reserved = YES
        //    and when deleting the reservation it should be set to reserved = NO
        //2 -  when we add new reservation we need if the room is already reserved
      ...
      public boolean addReservation(int client_id, int room_number, String dateIn, String dateOut )
    {
        ...
        try {
            ...
            {
                JOptionPane.showMessageDialog(null, "This Rooom is Already Reserve", "Room Reserved", JOptionPane.WARNING_MESSAGE);
                return false;
            }
    ...
    public boolean editReservation(int reservation_id, int client_id, int room_number,String dateIn, String dateOut)
    {
     ...
     // create a fuction to remove the selected reservation
    public boolean removeReservation(int reservation_id)
    {
        ...
        try {
            ...
            //we need to get the room number before daiting the resevation
